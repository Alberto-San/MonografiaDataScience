...
def get_outliers(current_class):
    ...
    plt.show()
    cp_command = "cp tmp/lof.csv tmp/lof_{}.csv".format(current_class)
    os.system(cp_command)

def filter_outliers(current_class):
    path = "tmp/lof_{}.csv".format(current_class)
...
original_data_without_outliers.to_csv("tmp/filtered_data.csv", index=False)
